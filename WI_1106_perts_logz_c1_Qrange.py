...
Qmax = 100000#150
# deltaQ = 28
numQ = 3#int((Qmax - Qmin)/deltaQ + 1)
Qval = np.logspace(np.log10(Qmin), np.log10(Qmax), numQ)#np.linspace(Qmin, Qmax, numQ)
...
plt.yscale("log")
plt.plot(Qval, y2horizon, '.', Qval, y2c0(Qval)*cgtr0fit(Qval, 2.5, 2.0, 2.8e-2, 6.8e-5) )

#%% Fit y2

popt, pcov = curve_fit(cgtr0fit, np.log10(Qval), np.log10(y2horizon/y2c0(Qval)), bounds=(0,3))

plt.xlabel("Q")
plt.ylabel("$\\langle\\hspace{0.2} y^2 \\hspace{0.1}\\rangle_*$")
plt.yscale("log")
plt.plot(Qval, y2horizon, '.', Qval, y2c0(Qval)*10**cgtr0fit(np.log10(Qval), *popt) )
...
int_file = path + 'results_c1_Q' + str(Qmin) + '-' + str(Qmax) + '_' + str(Nruns) + 'runs.txt'
...
path  = '/Users/vikasaragam/Documents/Python/'
int_file = path + 'results_c1_Q100.txt'
np.savetxt(int_file, data)

#%% Import horizon crossing data

path  = '/Users/vikasaragam/Documents/Python/'
file = 'results_c1_Q10-100000_1000runs.txt'#'results_c1_Q10-3000_1000runs.txt'
