...
Qmax = 150
# deltaQ = 28
numQ = 11#6#int((Qmax - Qmin)/deltaQ + 1)
...
def cless0fit(Q, alpha, beta, A, B):
    return (1 + A*Q**alpha)/(1 + B*Q**beta)

plt.xlabel("Q")
plt.ylabel("$\\langle\\hspace{0.2} y^2 \\hspace{0.1}\\rangle_*$")
plt.yscale("log")
plt.plot(Qval, y2horizon, '.', Qval, y2c0(Qval)*cless0fit(Qval, 0.2, 1.4, 0.78, 0.088) )

#%% Fit y2

popt, pcov = curve_fit(cless0fit, Qval, y2horizon/y2c0(Qval), bounds=(0.01,2))

plt.xlabel("Q")
plt.ylabel("$\\langle\\hspace{0.2} y^2 \\hspace{0.1}\\rangle_*$")
plt.yscale("log")
plt.plot(Qval, y2horizon, '.', Qval, y2c0(Qval)*cless0fit(Qval, *popt) )
...
plt.yscale("log")
plt.xlabel("z = k/(aH)")
plt.ylabel("$\\langle\\hspace{0.2} y^2 \\hspace{0.1}\\rangle$")
...
int_file = path + 'results_c-1_Q' + str(Qmin) + '-' + str(Qmax) + '_' + str(Nruns) + 'runs.txt'
...
path  = '/Users/vikasaragam/Documents/Python/'
int_file = path + 'results_c-1_Q100.txt'
np.savetxt(int_file, data)

#%% Import horizon crossing data

path  = '/Users/vikasaragam/Documents/Python/'
file = 'results_c-1_Q10-1400_1000runs.txt'#'results_c-1_Q10-150_1000runs.txt'#'results_c1_Q10-3000_1000runs.txt'
...
path  = '/Users/vikasaragam/Documents/Python/'
path_file = path + 'results_backgroundHc_constantQ_f1_m1e-2Mp_index_time_H_phidot_rhorad.txt'
